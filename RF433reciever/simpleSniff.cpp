...
void handleInterrupt() {
  ...
  //wait for lock
  if (lockChange == 0 && duration > 12000 && duration < 55000 && lockPassed == 0) { 
    changeCount = 0;
    ...
    lockChange = 1;
  } else if (lockChange == 1 && duration > 900 && duration < 1200 && lockPassed == 0) {
    log("Lock started 2"); cout << endl;
    log(measure2String(duration));cout << endl;
    durations[changeCount++] = duration;
    lockChange = 2;
  } else if (lockChange == 2 && duration > 950 && duration < 1200 && lockPassed == 0) {
    ...
    if (changeCount > MAX_CHANGE_TO_KEEP) { // store 100 change Max
      ...
      for (int i=3; i < MAX_CHANGE_TO_KEEP; i=i+2) {
        ...
        if (durations[i] > 900 && durations[i] < 1200 &&
            durations[i+1] > 900 && durations[i+1] < 1200  ) {
            //a 0
            cout << "0";
        } else if (durations[i] > 900 && durations[i] < 1200 &&
            durations[i+1] > 2000 && durations[i+1] < 2300  ) {
            //a 0
            cout << "1";
        } else {
          //errror reset
